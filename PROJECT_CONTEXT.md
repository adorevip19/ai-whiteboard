...
- 用题目图片 `/Users/tianyufeng/Desktop/我的装修/卫生间/截屏2026-05-06 11.47.29.png` 跑通了完整流程：图片识别、主讲解生成、脚本预检/修复、知识点卡片、专题讲解生成、TTS 抽样合成。
- 主讲解最终结果保存于 `/tmp/awb-main-response-final.json`：66 条命令、6 页，最终预检 0 错误 0 风险，知识点卡片同步返回。
- 专题讲解最终结果保存于 `/tmp/awb-topic-response-final.json`：64 条命令、5 页，最终预检 0 错误 0 风险。
- 专题生成过程中复现过一次 `Unterminated string in JSON`，当前服务端的无效 JSON 自动重试机制已兜住，最终没有向用户暴露失败。
- 本轮修复了服务端预检中 `write_paragraph` 参与重叠检查时过于保守的问题：现在使用实际换行后的文字足迹，而不是整块 slot 高度，避免把空白区域误判为文字重叠。
- 本轮新增生成脚本后的公式自动收口：`write_math` / `write_math_steps` 若靠右或靠下，会自动左移/上移，必要时缩小字号和行距，降低公式写出画布的风险。
- TTS 抽测主讲解和专题讲解各 3 段旁白，`/api/tts` 均返回 `audio/mpeg`，请求速率为 `rate: 1`。
- 知识点卡片定位调整为“干货速查卡”：只保留概念名、公式/定理、适用条件、关键原理和易错点；禁止写课堂话术、泛泛背景解释和长段落。
...
- `railpack.json` 的 `deploy.aptPackages` 安装 `chromium`、`ffmpeg`、`fonts-noto-cjk`，Railway/Railpack 运行时需要这些包完成 headless Chrome 录制、MP4 转码和中文字体渲染
- 如需用 Railway Variables 覆盖，设置 `RAILPACK_DEPLOY_APT_PACKAGES=chromium ffmpeg fonts-noto-cjk`
...
### 0. 2026-05-06 物理题端到端打磨记录

本轮用三张截图题做了“图片识别 → AI 生成脚本 → 预检/修复 → 服务端 MP4 渲染烟测”的完整回归：

- 钍234 半衰期题：最终脚本 4 页、60 条命令，预检通过，MP4 渲染成功。
- 双木块轻弹簧题：最终脚本 5 页、49 条命令，预检通过，MP4 渲染成功。
- 带电粒子垂直进入匀强磁场题：最终脚本 4 页、45 条命令，预检通过，MP4 渲染成功。

暴露并修复/补强的问题：

- AI 容易把物理公式塞进 `write_text`/`write_text_segments`，已在 `AI_GUIDE.md` 和 `server/aiScript.ts` 的生成提示中强化：物理公式、下标、分式、平方、单位换算和推导必须用 `write_math`/`write_math_steps`。
- AI 容易在每一页重复复制整套小物理图，导致页面过满，已强化短物理计算题建议：通常 `4–5` 页、`45–80` 条命令；完整图示只放在关系图页，推导页只保留公式或极简参照。
- `layout_page` slot 内手写标题再接 `write_paragraph(slotId)` 会导致重叠，已补充规则：标题用 `layout_page.title/subtitle`，或段落使用显式矩形从标题下方开始。
- 物理题中的“移动的距离”被旁白操作正则误判为白板操作播报，已调整预检正则：只拦截“移动到/移到/放到/拖到”等明显白板操作说法，不误伤物理概念。
- 带电粒子图中使用 `draw_point` 表示粒子/圆心不应触发“几何证明每页保留几何图”的预检规则，已调整为只有真正几何证明/构造命令才触发该类全局提醒。

本轮临时测试产物保存在 `/tmp/ai-whiteboard-physics-tests`，其中 `.script.json` / `.final.script.json` / `.repaired.script.json` 是最终验证脚本，`.smoke.mp4` 是服务端播放渲染烟测输出。
...
- Railway/生产环境需要同时具备 Chrome/Chromium、`ffmpeg` 和 CJK 中文字体；仓库 `railpack.json` 已设置 `deploy.aptPackages=["chromium","ffmpeg","fonts-noto-cjk"]`。如需用环境变量覆盖，应设置 `RAILPACK_DEPLOY_APT_PACKAGES=chromium ffmpeg fonts-noto-cjk`，并可设置 `CHROME_EXECUTABLE_PATH=/usr/bin/chromium`。
...
用户反馈物理位移图里出现夸张的巨大黑色大括号，遮住主要板书。根因是 `draw_brace` 在播放器里被渲染成超大字体 `{`/`}` glyph，横向跨度越长字号越大。

已处理：

- `ScriptRunner.animateBrace` 改为使用已有 `bracePath`，输出 `shapeType: "brace"` 的 SVG 线条路径，不再生成巨大字体 glyph。
- `AI_GUIDE.md` 明确：物理过程分段优先用彩色线段、箭头、段名标签和公式标签，避免用 `draw_brace` 横跨大半个图。
- `server/aiScript.ts` 基础指令同步加入该约束，减少后续生成类似问题。

## 2026-05-06 知识点卡片与专题讲解

用户希望每道题讲解涉及的基础知识、背景知识、概念、公式和原理可以随时查看，而不是只在播放结束时出现；如果学生听不懂当前题目，可以一键生成一个专门讲这些知识点的专题讲解，并能回到原题进度继续听。

已设计并实现：

- AI 生成脚本时同步返回 `knowledgeSummary`，不写入白板脚本 JSON，避免破坏旧脚本兼容。
- 新增 `/api/ai-script/knowledge-summary`，可从用户自加载或旧脚本里后台整理知识点。
- 白板页面右侧出现低干扰“知识点”入口，整理中/已准备/整理失败都有状态提示。
- 知识点卡片为覆盖式面板，可随时打开/隐藏，不挤压白板。
- 卡片中可以一键后台生成专题讲解；生成和 TTS 预缓存不打断当前题目播放。
- 专题准备好后用户手动切换；切换时保存原题 `scriptText`、步骤、TTS 和速度设置，专题听完后可返回原题继续播放。

当前限制：

- v1 只维护一层“原题 → 专题 → 返回原题”的关系，不支持专题里继续嵌套专题。
- 旧脚本知识点整理依赖 AI 接口；失败不影响播放，只显示可重试状态。
