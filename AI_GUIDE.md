...
**v1.6 更新**：新增 Azure TTS 旁白朗读、播放器速度调节和 `wait` 等待触发命令。AI 可以在关键讲解节点插入 `wait`，让白板停下来等待用户点击“下一步”后再继续。TTS 播放采用“段落首尾同步”：每条命令和它的旁白同时开始，白板按自己的动画速度绘制；如果白板先结束，会等当前旁白播完再进入下一条命令。
...
### 3.6 批注图层命令 — 划重点（手绘笔记风格）

批注命令写在一个**独立的透明图层**上，始终叠加在所有主画布元素之上。
批注笔迹具有手绘质感（轻微抖动、椭圆圈画有轻微形变），接近老师在白板上随手标注的效果。
